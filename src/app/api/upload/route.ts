...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const characterData of chunithmData.character || []) {
      // 즐겨찾기 여부 확인
      const isFavorite = chunithmData.favoriteCharacter && 
                          characterData.id === chunithmData.favoriteCharacter.id;
    ...
    // 모든 곡 점수 데이터 처리
    const allScores = [...(chunithmData.best || []), ...(chunithmData.new || []), ...chunithmData.score]
...
// 레이팅 계산 함수 (보면정수 기반)
function calculateRating(score: number, levelValue: Decimal): number {
  // 기준 점수에 따른 레이팅 계산
  let baseRating = 0;
  let additionalRating = 0;
  const baseConstant = Number(levelValue)

  // 점수별 레이팅 계산식 적용
  if (score >= 1009000) { // SSS+
    baseRating = baseConstant + 2.15;
  } 
  else if (score >= 1007500) { // SSS
    baseRating = baseConstant + 2.00;
    additionalRating = 0.01 * Math.floor((score - 1007500) / 100);
  }
  else if (score >= 1005000) { // SS+
    baseRating = baseConstant + 1.50;
    additionalRating = 0.01 * Math.floor((score - 1005000) / 50);
  }
  else if (score >= 1000000) { // SS
    baseRating = baseConstant + 1.00;
    additionalRating = 0.01 * Math.floor((score - 1000000) / 100);
  }
  else if (score >= 975000) { // S/S+
    baseRating = baseConstant + 0.00;
    additionalRating = 0.01 * Math.floor((score - 975000) / 250);
  }
  else if (score >= 925000) { // AA/AAA
    baseRating = baseConstant - 3.00;
    additionalRating = 0.03 * Math.floor((score - 925000) / 500);
  }
  else if (score >= 900000) { // A
    baseRating = baseConstant - 5.00;
    additionalRating = 0.01 * Math.floor((score - 900000) / 125);
  }
  else if (score >= 800000) { // BBB
    baseRating = (baseConstant - 5.00) * 0.5;
    additionalRating = 0.1 * (baseConstant - 5.00) * Math.floor((score - 800000) / 20000);
  }
  else if (score >= 600000) { // B/BB
    baseRating = 0.00;
    additionalRating = 0.1 * (baseConstant - 5.00) * Math.floor((score - 600000) / 60000);
  }
  else { // C/D
    baseRating = 0.00;
  }
